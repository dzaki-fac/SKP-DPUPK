...
export function SKPProvider({ children }: { children: ReactNode }) {
 ...
 if (!currentUser) return;
  ...
  // Target jumlah dan rincian terpisah — tidak auto. Target jumlah wajib selalu diisi, rincian opsional 0..5
  if (!planForm.target) { notify("Target jumlah wajib diisi"); return; }
 ...
 const parentTargets = (showCascadeModal as any).customTargets as Array<{name:string,value:string,unit:string}> ?? [];
 // Buat delegasi tanpa optimistic id — tunggu DB agar planId valid untuk realisasi turunan
 portions.forEach(({tid,val,title}) => {
 const payload: any = {
 parentId: showCascadeModal.id,
 skpPeriodId: showCascadeModal.skpPeriodId,
 ...
 plannedDate: (showCascadeModal as any).plannedDate ?? null,
 plannedTime: (showCascadeModal as any).plannedTime ?? null,
 log: false,
 };
 fetch("/api/plans", { method: "POST", headers: { "Content-Type": "application/json"}, credentials: "include", body: JSON.stringify(payload) })
 .then(async r=>{
 const j = await r.json().catch(()=>null);
 if(!r.ok){
 notify("Gagal simpan: "+(j?.error||r.statusText));
 return;
 }
 if (j && j.id) {
 setPlans(prev => {
 // hindari duplikat jika sudah ada (mis. dari /api/db refresh)
 if (prev.some(p=> p.id===j.id)) return prev;
 return [j, ...prev];
 });
 // refresh penuh untuk sinkron customTargets id & progress induk
 fetch("/api/db").then(rr=>rr.ok?rr.json():null).then(d=>{ if(d?.plans) setPlans(d.plans); }).catch(()=>{});
 }
 })
 .catch(() => notify("Gagal simpan pelimpahan ke database"));
 const emp = employees.find(e => e.id === tid);
 addLog("Pelimpahan kinerja", `Melimpahkan '${title}' (${val}) kepada ${emp?.name}`, "performance_plan", showCascadeModal.id);
 });
 ...
 if (realForm.files.length > 5) { notify("Maksimal 5 bukti per realisasi"); return; }
  // Validasi target terealisasi: warisi dari induk jika dilimpahkan agar tetap terbaca
  const getEffectiveForSubmit = (pid: string): Array<{name:string,value:string,unit:string}> => {
  const p = plans.find(x=>x.id===pid);
  if (!p) return [];
  if ((p as any).customTargets && (p as any).customTargets.length) return (p as any).customTargets;
  let cur: any = p;
  while (cur?.parentId) {
 ...
 const pKeys = realForm.participants.map(p=> `id:${p.employeeId}`);
 ...
 if (!p.employeeId) { notify("Pilih pegawai terlibat dari daftar"); return; }
 if (!p.role.trim() || p.role.trim().length > 30) { notify("Peran 1-30 karakter"); return; }
 }
 const r: Realization = { id: "r"+ Date.now(), planId: plan.id, title: titleTrim, value: String(valNum), description: realForm.description, date: dateVal, time: timeVal, uploadedBy: currentUser.id, targets: filteredTargetsSubmit.map(t=>({ id: "rt"+Date.now()+Math.random().toString(36).slice(2,5), name: t.name.trim(), value: t.value.trim(), unit: t.unit.trim() })), participants: realForm.participants.map(p=>({ id:"rp"+Date.now()+Math.random().toString(36).slice(2,5), employeeId: p.employeeId, customName: null, role:p.role.trim() } as any)) };
 ...
 return;
  ...
  // Validasi target terealisasi edit: warisi induk jika dilimpahkan
 ...
 if (realForm.participants.length > 10) { notify("Maksimal 10 pegawai terlibat"); return; }
 const updPKeys = realForm.participants.map(p=> `id:${p.employeeId}`);
 if (new Set(updPKeys).size !== updPKeys.length) { notify("Pegawai terlibat tidak boleh duplikat"); return; }
 for (const p of realForm.participants) {
 if (!p.employeeId) { notify("Pilih pegawai terlibat dari daftar"); return; }
 if (!p.role.trim() || p.role.trim().length > 30) { notify("Peran 1-30 karakter"); return; }
 }
 const updated: Realization = { ...editingRealization, title: titleTrim, description: realForm.description, date: dateVal, time: timeVal, targets: filteredTargetsEdit.map(t=>({ id: t.name+Date.now(), name: t.name.trim(), value: t.value.trim(), unit: t.unit.trim() })), participants: realForm.participants.map(p=>({ id:"rp"+Date.now()+Math.random().toString(36).slice(2,5), employeeId: p.employeeId, customName: null, role:p.role.trim() } as any)) };
