...
export default function OrgStructure() {
 ...
 <div
 key={n.id}
 className={`absolute bg-white border rounded-[10px] cursor-pointer transition-all ${dimNode(n) ? "opacity-30": " hover:border-[#3ba6f1] hover:shadow-sm"} ${detailId === n.id ? " border-[#3ba6f1] ring-1 ring-[#3ba6f1]/20": " border-[#e8e6e5] shadow-[0_1px_2px_rgba(17,24,39,0.04)]"}`}
 ...
 <div className={`${staf ? " w-6 h-6": " w-8 h-8"} rounded-full text-white flex items-center justify-center font-semibold leading-none shrink-0`} style={{ borderRadius: 9999, background: ROLE_ACCENT[n.role] || "#3ba6f1", fontSize: staf ? 9 : 11 }}>{n.avatar}</div>
 <div className="min-w-0 flex-1">
 <div className={`${staf ? " text-[12px]": " text-[12px]"} font-semibold text-[#0c0a09] truncate`}>{n.name}</div>
 <div className="text-[10px] text-[#a8a29e] truncate">{n.employeeNumber || " Tanpa NIP"}</div>
 </div>
 </div>
 {!staf && (
 <div className="mt-1.5 flex items-center gap-2 min-w-0">
 <span className="inline-flex items-center px-1.5 py-px rounded-full text-[10px] font-medium bg-[#c1e1f7] text-[#3ba6f1] border border-[#e8e6e5] shrink-0">{ROLE_SHORT[n.role]}</span>
 {(childrenOf.get(n.id)?.length ?? 0) > 0 && (
 <span className="inline-flex items-center gap-0.5 text-[10px] text-[#78716c] shrink-0">
 ...
 <div className="w-full max-w-md bg-white border border-[#e8e6e5] rounded-[10px] shadow-lg" onClick={e2 => e2.stopPropagation()}>
 <div className="px-6 py-5 flex items-center justify-between border-b border-[#fafaf9]">
 <div className="flex items-center gap-3">
 <div className="w-11 h-11 rounded-full text-white flex items-center justify-center text-[14px] font-semibold leading-none shrink-0" style={{ borderRadius: 9999, background: ROLE_ACCENT[e.role] || "#3ba6f1"}}>{e.avatar}</div>
 <div>
 <div className="font-semibold text-[14px] text-[#0c0a09]">{e.name}</div>
 <div className="text-[12px] uppercase tracking-[0.06em] text-[#3ba6f1]">{ROLE_SHORT[e.role]}</div>
 </div>
 </div>
 <button type="button" onClick={() => setDetailId(null)} className="w-8 h-8 rounded-full border border-[#e8e6e5] text-[#78716c] hover:border-[#3ba6f1] hover:text-[#0c0a09]" style={{ borderRadius: 9999 }}>✕</button>
 </div>
 <div className="px-6 py-5 space-y-4">
 {rows.map(r => (
 <div key={r.label}>
 <div className="text-[12px] uppercase tracking-[0.06em] text-[#a8a29e]">{r.label}</div>
 <div className="mt-1 text-[14px] text-[#0c0a09]">{r.value}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 {/* Area tree */}
 <div className="min-w-0 bg-white border border-[#e8e6e5] rounded-[10px] shadow-[0_1px_2px_rgba(17,24,39,0.04)]">
 <div className="px-4 py-3 border-b border-[#fafaf9] flex flex-wrap items-center justify-between gap-2">
 <span className="text-[12px] uppercase tracking-[0.07em] text-[#78716c]">Peta Hierarki Jabatan</span>
 <div className="flex items-center">
 <div className="relative w-full sm:w-auto">
 <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#a8a29e]" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
 <input
 value={search}
 onChange={e => setSearch(e.target.value)}
 placeholder="Cari nama, NIP, email, jabatan"
 className="w-[230px] pl-7 pr-3 py-1 rounded-md border border-[#e8e6e5] bg-white text-[12px] text-[#0c0a09] focus:outline-none focus:border-[#3ba6f1] focus:ring-1 focus:ring-[#3ba6f1]/20 placeholder:text-[#a8a29e]"
 />
 </div>
 </div>
 </div>
 ...
 {roots.length === 0 ? (
 <div className="px-4 py-14 text-center text-[#78716c] text-[14px]">Belum ada struktur organisasi.</div>
 ) : (
 <div className="overflow-x-auto">
 <div className="relative" style={{ width: totalW, height: totalH }}>
 <svg className="absolute inset-0" width={totalW} height={totalH} fill="none">
 <path d={connectorPath} stroke="#d6d3d1" strokeWidth={1.4} />
 </svg>
 ...
 {/* Panel info samping */}
 <aside className="bg-white border border-[#e8e6e5] rounded-[10px] p-4 shadow-[0_1px_2px_rgba(17,24,39,0.04)]">
 <div className="text-[12px] uppercase tracking-[0.07em] text-[#78716c]">Informasi Struktur</div>
 ...
 <div key={row.label} className="flex items-center justify-between py-1.5 border-b border-[#fafaf9] last:border-0">
 <span className="text-[12px] text-[#78716c]">{row.label}</span>
 <span className="text-[14px] font-semibold text-[#0c0a09]">{row.v}</span>
 </div>
 ))}
 </div>
 <div className="mt-4 pt-3 border-t border-[#fafaf9] flex items-center justify-between">
 <span className="text-[12px] font-semibold text-[#0c0a09]">Total Pegawai</span>
 <span className="text-[16px] font-medium text-[#0c0a09]">{treeNodes.length}</span>
 </div>
 <div className="mt-4 flex items-start gap-2 rounded-[10px] bg-[#c1e1f7] px-3 py-2.5">
 <svg className="mt-0.5 shrink-0 text-[#3ba6f1]" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
 <circle cx="12" cy="12" r="10"/>
 <path d="M12 16v-4"/>
 <path d="M12 8h.01"/>
 </svg>
 <p className="text-[12px] leading-relaxed text-[#0c0a09]">
