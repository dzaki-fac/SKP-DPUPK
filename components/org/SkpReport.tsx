  ...
  const rows = useMemo(() => {
    ...
    return accessible
      .map(p => {
        ...
        const aktif = getActiveSupervisors(p.assignedTo).map(s => s.supervisorId);
        const penilai = (aktif.length ? aktif : p.createdBy ? [p.createdBy] : [])
          .map(id => nameOf(id)?.name?.split(",")[0])
          .filter(Boolean)
          .join(", ");
